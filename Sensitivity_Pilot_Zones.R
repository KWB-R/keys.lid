...
library(GGally)
library(ggpubr)
setwd("Y:/WWT_Department/Projects/KEYS/Data-Work packages/WP1_sponge_city_elements/_DataAnalysis/LIDmodels/greenRoof/BWSTI_Zone1")

# read swmm input file
input <- read_inp(x = "BWSTI_Zone1.inp")
summary(input)
...
p1 <- ggplot(cal_results, aes(Soil_Thickness, Sum_R)) + geom_point(alpha=0.8, color='#0078d2') + ggtitle("Substrate Thickness") + theme_pubr() +
  ...
  stat_cor(label.y = 5100) + stat_regline_equation(label.y = 5300) + theme(plot.title = element_text(hjust=0.5, size=13))
...
fig2 <- annotate_figure(fig1, top = text_grob("Sensitivity Analysis - Zone 1 (Lhasa)", face = "bold", size = 15))
print(fig2)
